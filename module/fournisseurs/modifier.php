...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['modifier_fournisseur'])) {
        ...
        $updateQuery = $pdo->prepare("UPDATE fournisseurs SET nom = :nom, email = :email, telephone = :telephone WHERE id = :id");
        $updateQuery->execute([
            'nom' => $nom,
            'email' => $email,
            'telephone' => $telephone,
...
<h1>PARFUME DE FRANCE</h1>
    ...
    <div class="form-container">
        <form method="POST">
            ...
            <button type="submit" name="modifier_fournisseur">Mettre à jour</button>
        </form>
    </div>
</body>
</html>
